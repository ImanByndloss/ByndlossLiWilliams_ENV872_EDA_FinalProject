...
## Merging Species and Spatial Data Frames

This section merges both daily and monthly species data frames with the spatial data frame in preparation for the second round of ANOVAs that will be in the data analysis rmd file. The aforementioned second round of ANOVAs will attempt to determine which park attributes significantly influence any displayed differences in species activity between the three parks.

```{r}
# Rename column within combined data frame
# Filter rows of interest before merging
# Notably, chosen rows are according to unique differences in parks attributes
FilteredParkAttributes <- CombinedParkAttributes %>%
  dplyr::rename(ParkName = Name) %>%
  select(ParkName, YearEstabl, Acres, Hours, 
         Hiking, MountainBi, WalkJog, Camping, Fishing)
...
write.csv(CombinedParkAttributes, 
          row.names = FALSE, 
          file = "./Data/Processed/Park_Attributes.csv")

write.csv(DailyMerge, 
          row.names = FALSE, 
          file = "./Data/Processed/DailySpecies_ParkAttributes_Merge.csv")

write.csv(MonthlyMerge, 
          row.names = FALSE, 
          file = "./Data/Processed/MonthlySpecies_ParkAttributes_Merge.csv")
```
