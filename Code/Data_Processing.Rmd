...
BeamanSpeciesFullPeriodTotal <- data.frame(Species = character(), FullPeriodTotal = numeric())

BellsBendSpeciesResults <- data.frame()
BellsBendSpeciesMonthlyTotals <- data.frame()
BellsBendSpeciesFullPeriodTotal <- data.frame(Species = character(), FullPeriodTotal = numeric())

MillCreekSpeciesResults <- data.frame()
MillCreekSpeciesMonthlyTotals <- data.frame()
MillCreekSpeciesFullPeriodTotal <- data.frame(Species = character(), FullPeriodTotal = numeric())

# Load additional relevant package
library(zoo)
...
for (species in BeamanSpeciesList) {
  ...
  BeamanSpeciesData <- subset(BeamanID, BeamanID$species == species)
  ...
  group <- subset(BeamanSpeciesData, deployment.id == BeamanCameras[i])
  if(nrow(group) > 1){
  ...
  BeamanSpeciesData <- subset(BeamanFiltered, Independent == TRUE)
  ...
  for(i in 1:nrow(BeamanCameraDates)){
    group <- subset(BeamanSpeciesData, date == BeamanCameraDates[i,1])
    count <- nrow(group)
    CountRaw[i] <- count
  ...
  BeamanSpeciesFullPeriodTotal <- rbind(BeamanFullPeriodTotal, data.frame(Species = species, FullPeriodTotal = BeamanFullPeriodTotal))
}
...
for (species in BellsBendSpeciesList) {
  ...
  # Subset data for the current species
  BellsBendSpeciesData <- subset(BellsBendID, BellsBendID$species == species)
  ...
  group <- subset(BellsBendSpeciesData, deployment.id == BellsBendCameras[i])
  if(nrow(group) > 1){
    for(z in 1:((nrow(group))-1)){
      diff<-group[(z+1),3] - group[z,3]
      if(diff < filter){
  ...
  BellsBendSpeciesData <- subset(BellsBendFiltered, Independent == TRUE)
  ...
  for(i in 1:nrow(BellsBendCameraDates)){
    group <- subset(BellsBendSpeciesData, date == BellsBendCameraDates[i,1])
    count <- nrow(group)
    CountRaw[i] <- count
  ...
  # Store the full period total results for the current species
  BellsBendSpeciesFullPeriodTotal <- rbind(BellsBendFullPeriodTotal, data.frame(Species = species, FullPeriodTotal = BellsBendFullPeriodTotal))
}
...
for (species in MillCreekSpeciesList) {
  ...
  # Subset data for the current species
  MillCreekSpeciesData <- subset(MillCreekID, MillCreekID$species == species)
  ...
  group <- subset(MillCreekSpeciesData, deployment.id == MillCreekCameras[i])
  if(nrow(group) > 1){
    for(z in 1:((nrow(group))-1)){
      diff<-group[(z+1),3] - group[z,3]
      if(diff < filter){
  ...
  MillCreekSpeciesData <- subset(MillCreekFiltered, Independent == TRUE)
  ...
  for(i in 1:nrow(MillCreekCameraDates)){
    group <- subset(MillCreekSpeciesData, date == MillCreekCameraDates[i,1])
    count <- nrow(group)
    CountRaw[i] <- count
  ...
  # Store the full period total results for the current species
  MillCreekSpeciesFullPeriodTotal <- rbind(MillCreekFullPeriodTotal, data.frame(Species = species, FullPeriodTotal = MillCreekFullPeriodTotal))
}
```


## Preparing data before ANOVA -> NOT YET COMPLETE

This section includes...

```{r}
# Combine data into a long-format data frame
AllSpeciesData <- bind_rows(BeamanSpeciesResults, BellsBendSpeciesResults, MillCreekSpeciesResults)
