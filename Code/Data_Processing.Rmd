...
This section merges the daily and monthly species data frames with the park attributes data frame. These merges are completed in preparation for a second round of ANOVAs in the data analysis rmd file (which seeks to determine if the number of acres or allowance of disruptive activities influences species activity).
...
FilteredParkAttributes <- CombinedParkAttributes %>%
  dplyr::rename(ParkName = Name) %>%
   mutate(DisruptiveActivities = ifelse(MountainBi == "Yes" & 
                                          ...
                                          Fishing == "Yes", 
                                        "Yes", "No")) %>%
  select(ParkName, Acres, DisruptiveActivities)
