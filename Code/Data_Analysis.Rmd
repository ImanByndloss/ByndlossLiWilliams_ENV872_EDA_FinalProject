...
Monthly_Species_Count.data <- read.csv(
  file = here("./Data/Processed/Monthly_Species_Counts.csv"),
  stringsAsFactors = TRUE) 
```

#Two-way ANOVA Test - Daily Species Activity and Parks
...
#Two-way ANOVA Test - Monthly Species Activity and Parks

```{r, Second ANOVA, message = FALSE, warning=FALSE}
# Perform the two-way ANOVA with interaction
second_anova_result <- aov(CountAdjusted ~ ParkName * Species, data = Monthly_Species_Count.data)

# View the ANOVA summary
summary(second_anova_result)
```
...
# If the interaction term is significant, perform a post-hoc test
Monthly_Species_Count.data$ParkName <- as.factor(Monthly_Species_Count.data$ParkName)
Monthly_Species_Count.data$Species <- as.factor(Monthly_Species_Count.data$Species)

# Tukey's HSD test for the interaction term (ParkName * Species)
library(agricolae)
second_tukey_result <- TukeyHSD(second_anova_result, "ParkName:Species")

# Extract significant results
second_tukey_df <- as.data.frame(second_tukey_result$`ParkName:Species`)
second_tukey_df$Comparison <- rownames(second_tukey_result$`ParkName:Species`)
rownames(tukey_df) <- NULL

# Filter significant comparisons
second_significant_tukey <- subset(second_tukey_df, `p adj` < 0.05)

# Print the top 10 significant results
head(second_significant_tukey, 10)
...
ggplot(Monthly_Species_Count.data, aes(x=Species, y=CountAdjusted, fill=Species)) +geom_bar(stat = "identity") +facet_wrap(~ ParkName) + theme(legend.text = element_text(size = 6),        # Smaller legend text
