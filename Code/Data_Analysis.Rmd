...
```{r}
t_test_class <- t.test(CountAdjusted ~ Classification, data = Daily_Merge)
summary(t_test_class)
```

### Park Size (>1000ac vs <1000ac)
```{r}
t_test_size <- t.test(CountAdjusted ~ Size, data = Daily_Merge)
summary(t_test_class)
```

### Disruptive Activity, i.e., mountain biking, camping and fishing (Yes vs No)
```{r}
t_test_disrupt <- t.test(CountAdjusted ~ DisruptiveActivities, data = Daily_Merge)
summary(t_test_disrupt)
```
